...
public class DefaultScriptCamera : IScriptCamera
{
    ...
    public DefaultScriptCamera(int cameraId)
    {
        Id = cameraId;
    }
    ...
    {
        SetActiveWithInterpolation(Id, Alt.Natives.GetRenderingCam(), duration, easeLocation, easeRotation);
    }

    public void SetActiveWithInterpolation(IScriptCamera fromCamera, TimeSpan duration, bool easeLocation, bool easeRotation)
    {
    ...
    private static void SetActiveWithInterpolation(int toCamera, int fromCamera, TimeSpan duration, bool easeLocation, bool easeRotation)
    {
        Alt.Natives.SetCamActiveWithInterp(toCamera, fromCamera, (int)duration.TotalMilliseconds, easeLocation ? 1 : 0, easeRotation ? 1 : 0);
    }

    private static void Render(bool render, bool ease, TimeSpan easeTime)
    {
        Alt.Natives.RenderScriptCams(render, ease, (int)easeTime.TotalMilliseconds, false, false, 0);
    }
