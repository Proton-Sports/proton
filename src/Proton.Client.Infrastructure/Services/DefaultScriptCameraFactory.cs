...
public class DefaultScriptCameraFactory : IScriptCameraFactory
{
    ...
    public IScriptCamera CreateScriptCamera(ICameraHash cameraHash) =>
        CreateScriptCamera(cameraHash, false);

    public IScriptCamera CreateScriptCamera(ICameraHash cameraHash, bool active) =>
        new DefaultScriptCamera(Alt.Natives.CreateCamera(cameraHash.Hash, active));
}
